...
public class AreaDAO extends BaseDAO {
    ...
    public static AreaBean loadAreaById(int id) {
        ...
        return jdbi.withHandle(handle -> handle.createQuery("select id, user_id, type_area from " + TABLE_NAME + " where id = :id")
                .bind("id", id)
                .mapToBean(AreaBean.class).stream().findFirst().orElse(null));
    }

    public static AreaBean loadAreaByUserId(int id) {
        Jdbi jdbi = getJdbi();
        if (jdbi == null) {
            return null;
        }
        return jdbi.withHandle(handle -> handle.createQuery("select id, user_id, type_area from " + TABLE_NAME + " where user_id = :id")
                .mapToBean(AreaBean.class).stream().findFirst().orElse(null));
    }
    ...
    public static AreaBean loadAreaCommon(String typeArea) {
        ...
        return jdbi.withHandle(handle -> handle.createQuery("select id, user_id, type_area from " + TABLE_NAME + " where user_id = -1 and type_area = :typeArea")
