...
export class PickCharacterScene extends AbsScene {
  ...
  handleResLoadCharacters(resLoadCharacters: proto.IResLoadCharacters) {
    if (
      resLoadCharacters.character == null ||
      resLoadCharacters.character.length == 0
    ) {
  ...
  onClickConfirmPickCharacter() {
    ...
    AudioManger.me().playOneShot(AUDIOS.CLICK_3);
    DataSender.sendReqPickCharacter(
      Number.parseInt(this.characterPicked),
      this.editboxNamePlayer.string
    );
  }
    ...
    if (this.characterPanel) {
      ...
      this.characterPanel.children.forEach((node) => {
        ...
        if (characterCompoponent.getCharacterID() == characterId) {
          ...
          characterCompoponent.getCharacterBG().getComponent(Sprite).grayscale =
            false;
          characterCompoponent
            .getCharacterSprite()
            .getComponent(Sprite).grayscale = false;
          tween(node)
  ...
  resetGrayscale(nodeReset: Node) {
    if (this.characterPanel) {
      this.characterPanel.children.forEach((node) => {
        ...
        characterCompoponent
          .getCharacterSprite()
          .getComponent(Sprite).grayscale = true;
