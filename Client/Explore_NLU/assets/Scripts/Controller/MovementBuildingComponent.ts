...
export class MovementBuildingComponent extends Component {
  ...
  protected onLoad(): void {
    ...
    this.node.on(
      ...
      this.handleOpenMenuToolBuilding,
      this
    );
  }
  ...
  private handleOpenMenuToolBuilding(): void {
    ...
    this.menuToolBuilding.active = true;
    this.menuToolBuilding.setPosition(
      this.node.getPosition().x,
      this.node.getPosition().y - 115,
      0
    );
    this.buttonMove = this.menuToolBuilding
      .getChildByName("MenuToolContent")
      .getChildByName("ButtonMove");
  ...
  private handleMoveLand(event: EventTouch): void {
    ...
    if (this.node.parent == null) {
      newLocation = this.node
        .getComponent(UITransform)
        .convertToNodeSpaceAR(worldPos);
    } else {
      newLocation = this.node.parent
        .getComponent(UITransform)
        .convertToNodeSpaceAR(worldPos);
    }
  ...
  private handleTouchCoating(): void {
    if (this.menuToolBuilding) this.menuToolBuilding.active = false;
    CoatingComponent.me()
      .getCoating(COATING.MOVE)
      .off(Node.EventType.TOUCH_START, this.handleTouchCoating, this);
